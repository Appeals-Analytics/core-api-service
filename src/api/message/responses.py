...
class MessageResponse(BaseModel):
    ...
    def sentiment_label_ru(self) -> str:
        return SENTIMENT_TRANSLATIONS.get(
            self.sentiment_label, self.sentiment_label.value
        )
    ...
    def category_level_1_ru(self) -> str:
        return CATEGORY_LEVEL_1_TRANSLATIONS.get(
            self.category_level_1, self.category_level_1.value
        )
    ...
    def category_level_2_ru(self) -> List[str]:
        return [
            CATEGORY_LEVEL_2_TRANSLATIONS.get(cat, cat.value)
            for cat in self.category_level_2
        ]
