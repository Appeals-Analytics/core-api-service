...
class FileProcessor(ABC):
  ...
  def validate_structure(self, df: pl.DataFrame) -> None:
    ...
    if missing:
      raise ValueError(
        f"Отсутствуют обязательные колонки: {', '.join(missing)}. Пожалуйста, проверьте структуру файла."
      )
...
class CSVProcessor(FileProcessor):
  def read_data(self) -> pl.DataFrame:
    try:
      ...
      raise ValueError(
        f"Ошибка чтения CSV файла: {e}. Убедитесь, что файл имеет корректный формат CSV."
      )
...
class FileProcessorFactory:
  ...
  def create(cls, file_path: Path) -> FileProcessor:
    ...
    if not processor:
      raise ValueError(
        f"Неподдерживаемый формат файла: {ext}. Поддерживаемые форматы: {', '.join(cls._processors.keys())}"
      )
